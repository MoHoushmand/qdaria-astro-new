...
const MARKET_SALARY_BY_TIER: Record<
  string,
  { stage: string; values: [number, number, number, number] }[]
> = {
  founder: [
 ...
 */
export async function generateContractPdfForEmployee(
  opts: PdfOptions,
): Promise<void> {
  ...
  const equityPercentage =
    overrides?.equityPercentage ?? member.equity_percentage ?? 0;
  ...
  if (alloc) {
    ...
    for (const id of orderedIds) {
      ...
      if (pct !== undefined && pct > 0)
        equityRows.push({ company: idToName[id] || id, pct });
    }
  } else {
  ...
  const enabledIds =
    enabledClauseIds ??
    new Set(employmentContractTemplate.clauses.map((c) => c.id));
  const activeClauses = employmentContractTemplate.clauses.filter((c) =>
    enabledIds.has(c.id),
  );
  ...
  const templateValues = {
    ...
    seedSalary: salaryEntry
      ? formatNok(salaryEntry.salaries["seed"])
      : formatNok(Math.round(salaryEur * 1.25)),
    seriesASalary: salaryEntry
      ? formatNok(salaryEntry.salaries["round-a"])
      : formatNok(Math.round(salaryEur * 1.6)),
    seriesBSalary: salaryEntry
      ? formatNok(salaryEntry.salaries["round-b"])
      : formatNok(Math.round(salaryEur * 2.0)),
    seriesCSalary: salaryEntry
      ? formatNok(salaryEntry.salaries["round-c"])
      : formatNok(Math.round(salaryEur * 2.5)),
    ipoSalary: salaryEntry
      ? formatNok(salaryEntry.salaries["ipo"])
      : formatNok(Math.round(salaryEur * 3.0)),
  ...
  // --- COO appendix ---
  let getCooAppendixForEmployee: ((name: string) => string | null) | null =
    null;
  try {
    ...
    for (const line of lines) {
      ...
      if (/^\d+\.\s+[A-Z][A-Z\s&,/]+$/.test(line.trim())) {
        ...
        doc.line(
          x,
          y,
          x + Math.min(doc.getTextWidth(line.trim()) + 4, maxW),
          y,
        );
  ...
  const renderTable = (
    ...
    for (let ri = 0; ri < rows.length; ri++) {
      ...
      if (isTotal) doc.setFillColor(235, 240, 248);
        ...
        doc.setFillColor(STRIPE_ODD.r, STRIPE_ODD.g, STRIPE_ODD.b);
  ...
  const renderBarChart = (
    ...
    for (let s = 0; s <= 5; s++) {
      ...
      doc.text(sv >= 1000 ? `${Math.round(sv / 1000)}k` : String(sv), sx, y, {
        align: "center",
      });
    ...
    for (let i = 0; i < labels.length; i++) {
      ...
      doc.setTextColor(NAVY.r, NAVY.g, NAVY.b);
      doc.text(
        `${unit} ${formatNok(values[i])}`,
        Math.min(cax + bw + 2, cax + caw - 5),
        by + 5.5,
      );
    }
  ...
  const renderGroupedBarChart = (
    ...
    for (let i = 0; i < labels.length; i++) {
      ...
      doc.text(
        ...
        Math.min(cax + bw1 + 2, cax + caw - 5),
        by + 4,
      );
      doc.setTextColor(MID_GRAY.r, MID_GRAY.g, MID_GRAY.b);
      doc.text(
        `${unit ? unit + " " : ""}${formatNok(values2[i])}`,
        Math.min(cax + bw2 + 2, cax + caw - 5),
        by + barH + 5,
      );
    }
  ...
  const renderStackedBar = (
    ...
    for (const seg of segments) {
      ...
      if (segW > 12) {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(255, 255, 255);
        doc.text(pctStr, bx + segW / 2, y + barH / 2 + 1.5, {
          align: "center",
        });
      }
      bx += segW;
  ...
  for (const entry of tocEntries) {
    ...
    if (le > ls + 10) {
      doc.setDrawColor(LIGHT_GRAY.r, LIGHT_GRAY.g, LIGHT_GRAY.b);
      doc.setLineWidth(0.3);
      for (let dx = ls; dx < le; dx += 2.5)
        doc.line(dx, y - 0.5, dx + 1, y - 0.5);
    }
    y += 5.5;
  }
  y += 5;
  ...
  for (const clause of filledClauses) {
    ...
    if (eqParts.length > 1) {
      ...
      const eqData = equityRows.map((r) => {
        ...
        return [
          r.company,
          `${r.pct.toFixed(2)}%`,
          isHolding ? shareType : "Milestone",
          isHolding ? "Immediate" : triggerRound,
        ];
      });
    ...
    if (salParts.length > 1) {
      ...
      const roundMeta: {
        key: (typeof fundingRounds)[number];
        label: string;
        target: string;
      }[] = [
      ...
      for (let ri = 0; ri < roundMeta.length; ri++) {
        ...
        if (ri === 0) {
          ...
          const pct =
            prevSal > 0 ? Math.round(((currSal - prevSal) / prevSal) * 100) : 0;
          increaseStr = pct > 0 ? `+${pct}%` : `${pct}%`;
        }
        salData.push([
          rm.label,
          rm.target,
          currSal ? `EUR ${formatNok(currSal)}` : "\u2014",
          increaseStr,
        ]);
        prevSal = currSal;
      }
      renderTable(
        "Salary Progression by Funding Stage",
        salHeaders,
        salData,
        salColWidths,
      );
    ...
    if (fbParts.length > 1) {
      ...
      renderTable(
        "Funding Milestone Bonus Schedule",
        fbHeaders,
        fbData,
        fbColWidths,
      );
    ...
    const msbParts = remaining.split(
    ...
    if (msbParts.length > 1) {
      ...
      const tierKey = tierLower.includes("founder")
        ? "founder"
        : ["c-suite", "coo", "cto", "cfo", "cmo"].includes(tierLower)
          ? "c-suite"
          : ["senior", "leadership"].includes(tierLower)
            ? "senior"
            : ["mid", "specialist"].includes(tierLower)
              ? "mid"
              : "junior";
      const benchmarks =
        MARKET_SALARY_BY_TIER[tierKey] || MARKET_SALARY_BY_TIER["mid"];
      const msbHeaders = [
        "Stage",
        `QDaria (${title})`,
        "Market Low",
      ...
      const msbData = benchmarks.map((b) => {
        const vsMedian =
          b.values[2] > 0
            ? Math.round(((b.values[0] - b.values[2]) / b.values[2]) * 100)
            : 0;
      ...
      renderTable(
        `Market Salary Comparison - ${title}`,
        msbHeaders,
        msbData,
        msbColWidths,
      );
      ...
      renderGroupedBarChart(
        ...
        benchmarks.map((b) => b.values[0]),
        "QDaria",
        CYAN,
        benchmarks.map((b) => b.values[2]),
        "Market Median",
        { r: 180, g: 180, b: 180 },
        "EUR",
      );
    ...
    if (budParts.length > 1) {
      ...
      budData.push([
        "TOTAL",
        `EUR ${formatNok(seedBudgetBreakdown.reduce((s, b) => s + b.totalEur, 0))}`,
        "100%",
      ]);
      renderTable(
        `Seed Budget Allocation — €${(seedFundingTarget / 1_000_000).toFixed(0)}M over ${seedRunwayMonths} months`,
        budHeaders,
        budData,
        budColWidths,
      );
    ...
    if (sfParts.length > 1) {
      ...
      renderTable(
        "Spinoff Company Funding Targets",
        sfHeaders,
        sfData,
        sfColWidths,
      );
    ...
    if (capParts.length > 1) {
      ...
      renderTable(
        "QDaria Holdings Cap Table (Fully Diluted)",
        capHeaders,
        capData,
        capColWidths,
      );
    ...
    if (sevParts.length > 1) {
      ...
      renderTable(
        "Severance and Termination Terms by Tier",
        sevHeaders,
        sevData,
        sevColWidths,
      );
    ...
    if (ncParts.length > 1) {
      ...
      renderTable(
        "Non-Compete and Non-Solicitation (Norwegian Law)",
        ncHeaders,
        ncData,
        ncColWidths,
      );
    ...
    if (sotParts.length > 1) {
      ...
      renderTable(
        "Norwegian Stock Option Tax Incentive (March 2025)",
        sotHeaders,
        sotData,
        sotColWidths,
      );
    ...
    if (vnParts.length > 1) {
      ...
      renderTable(
        "Vesting Schedule Norms (Market Comparison)",
        vnHeaders,
        vnData,
        vnColWidths,
      );
  ...
  doc.setTextColor(MID_GRAY.r, MID_GRAY.g, MID_GRAY.b);
  doc.text(
    `Equity allocation for ${employeeName} across QDaria Holdings and spin-off companies`,
    margin,
    y + 4,
  );
  y += 10;
  ...
  renderStackedBar(
    ...
    equityRows.map((r, i) => ({
      label: r.company
        .replace("QDaria Holdings AS", "Holdings")
        .replace(/\s+AS$/, ""),
      value: r.pct,
      color: SEGMENT_COLORS[i % SEGMENT_COLORS.length],
    })),
  ...
  doc.setTextColor(MID_GRAY.r, MID_GRAY.g, MID_GRAY.b);
  doc.text(
    "Indicative annual base salary (EUR) per funding round, subject to Board approval",
    margin,
    y + 4,
  );
  y += 10;
  ...
  const sR: string[][] = fundingRounds.map((round) => {
    ...
    return [
      fundingRoundLabels[round],
      sal ? `EUR ${formatNok(sal)}` : "-",
      round === "pre-seed" ? "Current" : "Projected",
    ];
  });
  ...
  const stageValuations = [
    2000000, 12000000, 40000000, 120000000, 300000000, 1000000000,
  ];
  ...
  doc.setTextColor(MID_GRAY.r, MID_GRAY.g, MID_GRAY.b);
  doc.text(
    "One-time bonuses triggered upon successful completion of each funding round",
    margin,
    y + 4,
  );
  y += 10;
  ...
  doc.setTextColor(MID_GRAY.r, MID_GRAY.g, MID_GRAY.b);
  const bonusNote =
    "Note: The Employee must be actively employed at the time of the triggering event. Salary bonuses are paid within 30 days. Equity acceleration applies to total unvested equity across all QDaria companies.";
  ...
  doc.setTextColor(MID_GRAY.r, MID_GRAY.g, MID_GRAY.b);
  doc.text(
    "Industry sources used to benchmark compensation and equity allocations",
    margin,
    y + 4,
  );
  y += 10;
  ...
  if (boardRole) {
    ...
    const compensationText =
      "Compensation for Board membership duties is included in the employment compensation package detailed in this Agreement. No separate board fees shall be payable unless otherwise resolved by the General Meeting of Shareholders.";
  ...
  if (false) {
    ...
    if (cooText) {
      ...
      if (cooEqParts.length > 1) {
        ...
        const cooEqData = equityRows.map((r) => [
          r.company,
          `${r.pct.toFixed(2)}%`,
        ]);
        cooEqData.push(["TOTAL", `${equityTotal.toFixed(2)}%`]);
        renderTable(
          "COO Equity Allocation",
          cooEqHeaders,
          cooEqData,
          cooEqColWidths,
        );
      ...
      if (cooSalParts.length > 1) {
        ...
        const baseSal = salaryEntry
          ? salaryEntry.salaries["pre-seed"]
          : salaryEur;
    ...
    doc.setTextColor(MID_GRAY.r, MID_GRAY.g, MID_GRAY.b);
    doc.text(
      `Compensation rationale for ${employeeName} — ${tier} tier`,
      margin,
      y + 4,
    );
    y += 12;
  ...
  if (employeeName === "Daniel Mo Houshmand") {
    ...
    doc.setTextColor(MID_GRAY.r, MID_GRAY.g, MID_GRAY.b);
    doc.text(
      "Multi-role compensation rationale — Daniel Mo Houshmand",
      margin,
      y + 4,
    );
    y += 12;
