---
title: "156-Qubit Quantum Reservoir Computing on Real Hardware: A Sample Efficiency Crisis"
...
We ran quantum reservoir computing across all 156 qubits of IBM's Heron r2 processor (ibm_fez, open plan), which, to our knowledge, is a larger qubit count than any previously reported QRC hardware experiment. The larger reservoir did not predict better. But the most important discovery was not about scale. It was about a fundamental crisis that changes how we must think about quantum machine learning.
...
With 4 qubits producing 10 features (after our feature engineering), we achieve 5.0 samples per feature. This is a comfortable operating point. The classical readout layer has plenty of examples to learn from.

### System 2: Large-Scale IBM Heron r2 (156 Qubits)

Our second system scaled to IBM's full 156-qubit Heron r2 processor with 200 training samples. 

The Heron r2 is IBM's 156-qubit Heron generation of processors, featuring improved coherence times and gate fidelities compared to earlier generations. We chose this system specifically to push the boundaries of what is possible on current hardware.
...
Our third system used a high-fidelity simulation of Rigetti's Novera processor with 800 samples, 640 of them for training. We applied the full Steinegger-Räth feature engineering methodology, expanding the 9-qubit measurements to 3,375 features.
...
|--------|--------|------------------|----------|-----------------|----------|
| IBM Small | 4 | 50 | 10 | 5.0 | IBM Quantum |
...
The 4-qubit system achieved $R^2 = 0.764$. This is solid performance. The quantum reservoir successfully captures the dynamics of the spectral data and produces accurate forecasts.
...
|--------|--------|-------------|----------------|------------------|-------------|
| IBM Small | 4 | 0.764 | 0.18 | 0.22 | Low |
...
Our 4-qubit system: $50 / 10 = 5.0$  -  Safe.
...
<Figure
  ...
  caption="Illustrative noise-variation map for a Heron-class processor. Calibration was not recorded during our runs; this figure is schematic, not measured."
/>
...
Calibration data for our run days were not recorded, so we cannot report per-qubit error rates for the circuits we executed. The figure above is a schematic of the kind of variation IBM publishes for Heron-class processors, where some qubits sit below 0.1% error and others above 1%; any such variation affects different qubit subsets differently.
...
We have presented a full-processor 156-qubit QRC experiment on IBM Heron r2 hardware (ibm_fez), with job identifiers and reservoir states published alongside the code. But our key contribution is not the scale. It is the discovery of a fundamental sample efficiency crisis.
...
<ListOfFigures figures={[
  ...
  { number: 9, caption: "Illustrative noise-variation map (schematic, not measured)" },
